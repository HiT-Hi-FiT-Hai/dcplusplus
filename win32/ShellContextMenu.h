...
public:
	...
	UINT ShowContextMenu(typename T::WidgetMenuPtr menu, T * mainWindow, POINT pt)
	{
		...
		{
			OldWndProc = (WNDPROC) SetWindowLong(mainWindow->handle(), GWL_WNDPROC, (DWORD) HookWndProc);
			if(iMenuType == 2)
		...
		UINT idCommand = menu->trackPopupMenu(mainWindow, pt.x, pt.y, TPM_RETURNCMD | TPM_LEFTALIGN);

		if(OldWndProc) // unsubclass
			SetWindowLong(mainWindow->handle(), GWL_WNDPROC, (DWORD) OldWndProc);
